...
// 교수님만 보이는 "그룹 개설" 다이얼로그. 학생 계정에는 아예 렌더링하지 않습니다.
...
function GroupsContent() {
  ...
  // 참여 신청 중인 그룹이 있을 때만 화면을 좌/우로 분할합니다.
  ...
  // [신규] 로컬 환경에서는 초대 "링크"가 도메인/포트 문제로 잘 안 열릴 수 있어서,
  // 링크 없이도 코드만 직접 입력해서 참여 신청할 수 있는 방법을 추가합니다.
    ...
    <div className='min-h-screen w-full bg-[#FCFCFC] font-sans text-[#173A23]'>
      ...
      <main className='max-w-7xl mx-auto space-y-6 p-4 md:p-6'>
        ...
        <div className='flex flex-col sm:flex-row items-center justify-between gap-3'>
          ...
          {/* [수정] 교수님은 그룹 개설, 학생은 초대 코드 입력 - 역할별로 이 자리를 다르게 씁니다 */}
          {me?.role === 'professor' ? (
        ...
        {isGroupsLoading ? (
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5'>
            {filteredGroups.map((group: GroupItem) => {
              ...
              const memberCount = group.members_count ?? Math.max((group.members?.length ?? 0) - 1, 0) // 오너(교수님)도 GroupMember로 저장되므로 학생 수를 셀 땐 1명 빼줍니다.
        ...
        {/* 참여 신청 중인 그룹 (오른쪽 패널) - 신청한 그룹이 있을 때만 노출 */}
        {hasPendingInvites && (
